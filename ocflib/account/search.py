...
def users_by_filter(ldap_filter):
    ...
    with ldap.ldap_ocf() as c:
        c.search(
            OCF_LDAP_PEOPLE, ldap_filter, attributes=("uid",), search_scope=ldap3.LEVEL,
        )
...
def users_by_calnet_uid(calnet_uid):
    ...
    return users_by_filter("(calnetUid={})".format(escape_filter_chars(calnet_uid)))


def users_by_callink_oid(callink_oid):
    """Get a list of users associated with a CalLink OID"""
    callink_oid = int(callink_oid)
    return users_by_filter("(callinkOid={})".format(escape_filter_chars(callink_oid)))
...
def user_attrs_ucb(uid):
    return user_attrs(uid, connection=ldap.ldap_ucb, base=UCB_LDAP_PEOPLE)
