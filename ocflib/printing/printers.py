...
def _snmp(host, oid):
    err_indication, err_status, err_idx, response_kv = cmdgen.CommandGenerator().getCmd(
    ...
    if err_indication:
        ...
        raise IOError("Device {} returned error status: {}".format(host, err_status),)
    elif isinstance(response_kv[0][1], NoSuchObject):
        raise IOError("Device {} returned error status: NoSuchObject()".format(host),)
    else:
...
def get_toner(printer):
    ...
    return tuple(int(_snmp(printer, oid)) for oid in (OID_TONER_CUR, OID_TONER_MAX))


def get_maintkit(printer):
    """Returns (cur, max) maintenance kit tuple for the given printer."""
    return tuple(
        int(_snmp(printer, oid)) for oid in (OID_MAINTKIT_CUR, OID_MAINTKIT_MAX)
    )
