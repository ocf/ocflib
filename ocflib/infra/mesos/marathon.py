...
class MarathonClient:
    ...
    def request(self, method, path, *args, expected_status=200, **kwargs):
        req = requests.request(
            method, self.url + path, auth=(self.user, self.password), *args, **kwargs,
        )
    ...
    def deploy_app(
        self, app, new_config, report=_noop, force=False, timeout=600,
    ):
        ...
        for _ in range(timeout):
            ...
            if not status["app"]["deployments"]:
                ...
                report(
                    "Waiting for deployment to finish: {}".format(
                        status["app"]["deployments"],
                    ),
                )
                time.sleep(1)
        else:
            ...
            raise DeploymentException(
                ...
                "Automatically rolling back.".format(bad_deployment, timeout,),
            )

    def deploy_new_version(self, app, version, report=_noop, **kwargs):
        ...
        self.deploy_app(
            app, {"container": status["app"]["container"]}, report=report, **kwargs,
        )
