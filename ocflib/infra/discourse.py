...
class DiscourseTopic(
    namedtuple("DiscourseTopic", ("number", "title", "starter", "category")),
):
    ...
    def from_number(cls, api_key, num):
        ...
        topic_resp = requests.get(
            "{}/t/{}.json".format(DISCOURSE_ROOT, num), params=params, timeout=10,
        )
        if topic_resp.status_code != 200:
            raise DiscourseError("Topic request gave {}".format(topic_resp.status_code))
        ...
        cat_resp = requests.get(
            "{}/categories.json".format(DISCOURSE_ROOT), params=params, timeout=10,
        )
