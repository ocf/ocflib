...
class TestCreateLdapEntry:
    def test_normal_creation(
        self, mock_subprocess_check_output, mock_send_problem_report,
    ):
        create_ldap_entry(
            "uid=ckuehl,ou=People,dc=OCF,dc=Berkeley,dc=EDU", {"a": ["b", "c"]},
        )
    ...
    def test_normal_creation_keytab(
        self, mock_subprocess_check_output, mock_send_problem_report,
    ):
        create_ldap_entry(
            "uid=ckuehl,ou=People,dc=OCF,dc=Berkeley,dc=EDU",
            {
                ...
                "d": 12,
                "e": datetime(
                    2016, 11, 5, 12, 0, 0, tzinfo=timezone(timedelta(hours=-7)),
                ),
            },
            keytab="/nonexist",
    ...
    def test_create_existing(
        ...
        mock_subprocess_check_output.side_effect = CalledProcessError(
            68, "cmd", output="Already exists (68)",
        )
    ...
    def test_unexpected_error(
        ...
        mock_subprocess_check_output.side_effect = CalledProcessError(
            35, "cmd", output="lol random error",
        )
        with pytest.raises(ValueError) as error:
            create_ldap_entry(
                "uid=ckuehl,ou=People,dc=OCF,dc=Berkeley,dc=EDU",
...
class TestModifyLdapEntry:
    ...
    def test_normal_modification_keytab(
        self, mock_subprocess_check_output, mock_send_problem_report,
    ):
        modify_ldap_entry(
            "uid=mattmcal,ou=People,dc=OCF,dc=Berkeley,dc=EDU",
    ...
    def test_modify_nonexistent(
        ...
        mock_subprocess_check_output.side_effect = CalledProcessError(
            32, "cmd", output="No such object (32)",
        )
        with pytest.raises(ValueError) as error:
            modify_ldap_entry(
