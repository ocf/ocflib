...
def mock_modify_ldap_entry():
    with mock.patch("ocflib.infra.ldap.modify_ldap_entry",) as modify_ldap_entry:
...
class TestChangePasswordWithStaffer:
    def _chpass(self, mock_spawn):
        change_password_with_staffer(
            "ggroup", "strong_hunter2", "ckuehl/admin", "super_hunter2",
        )
        mock_spawn.assert_called_with(
            "/usr/bin/kadmin -p ckuehl/admin cpw ggroup", timeout=10,
        )
    ...
    def test_kadmin_failure(
        self, error, mock_spawn, mock_notify_password_change,
    ):
...
class TestChangePasswordWithKeytab:
    def _chpass(self, mock_spawn):
        change_password_with_keytab(
            "ggroup", "strong_hunter2", "/some/keytab", "create/admin",
        )
        mock_spawn.assert_called_with(
            "/usr/bin/kadmin -K /some/keytab -p create/admin cpw ggroup", timeout=10,
        )
        mock_spawn.return_value.sendline.assert_has_calls(
            [mock.call("strong_hunter2"), mock.call("strong_hunter2")],
        )
...
class TestModifyLdapAttributes:
    ...
    @pytest.mark.parametrize(
        "attrs", [{"loginShell": "/bin/bush"}, {"loginShell": ""},],
    )
    def test_invalid_attributes(self, attrs, mock_modify_ldap_entry):
        with pytest.raises(ValueError):
            modify_ldap_attributes(
                "ggroup", attrs, keytab="/some/keytab", admin_principal="create/admin",
            )
