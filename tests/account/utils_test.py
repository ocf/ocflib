...
class TestPasswordMatches:
    ...
    def test_calls_pexpect_correctly(self, spawn, __):
        ...
        spawn.assert_called_with(
            "kinit --no-forwardable -l0 ckuehl@OCF.BERKELEY.EDU", timeout=10,
        )
...
class TestUserPaths:
    @pytest.mark.parametrize(
        "user,expected",
        [("ckuehl", "/home/c/ck/ckuehl"), ("ggroup", "/home/g/gg/ggroup"),],
    )
    def test_home_dir(self, user, expected):
        assert home_dir(user) == expected

    @pytest.mark.xfail
    @pytest.mark.parametrize("user", ["", "c", "ck",])
