...
@pytest.mark.parametrize("uid,users", [(872544, ["daradib"]), (666666, []),])
def test_users_by_calnet_uid(uid, users):
    assert users_by_calnet_uid(uid) == users


@pytest.mark.parametrize("oid,users", [(46130, ["bpreview"]), (666666, []),])
...
@pytest.mark.parametrize(
    "user,exists", [("ckuehl", True), ("bpreview", True), ("doesnotexist", False),],
)
def test_user_exists(user, exists):
    assert user_exists(user) == exists


@pytest.mark.parametrize("user,sorried", [("ckuehl", False), ("ofc", True),])
def test_user_is_sorried(user, sorried):
    assert user_is_sorried(user) == sorried


class TestUserIsGroup:
    @pytest.mark.parametrize("user,exists", [("ckuehl", False), ("bpreview", True),])
