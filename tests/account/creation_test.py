...
class TestCreateDirectories:
    ...
    def test_ensure_web_dir_existing(self):
        with mock.patch("os.path.exists", return_value=True), mock.patch(
            "os.path.realpath", return_value="/services/http/users/c/ckuehl",
    ...
    def test_ensure_web_dir_rename(self):
        with mock.patch("os.path.exists", return_value=True), mock.patch(
            "os.path.realpath", return_value="/home/c/ck/ckuehl/public_html",
        ), mock.patch("subprocess.check_call") as check_call, freeze_time(
            "2015-08-22 14:11:44",
        ):
            ensure_web_dir("ckuehl")
            check_call.assert_has_calls(
...
class TestUsernameBasedOnRealName:
    ...
    def test_some_names(
        self, _, __, username, realname, success,
    ):
    ...
    @pytest.mark.parametrize("username", ["shitup", "ucbcop", "suxocf",])
    @mock.patch("ocflib.account.search.user_exists", return_value=False)
    ...
    @pytest.mark.parametrize("username", ["wordpress", "systemd", "ocf", "ocfrocks",])
    @mock.patch("ocflib.account.search.user_exists", return_value=False)
    @mock.patch("ocflib.account.creation.similarity_heuristic", return_value=0)
    ...
    def test_long_names(self, _, __):
        ...
        with pytest.raises(ValidationWarning):
            ...
            validate_username(
                "nomatch", "I Have Sixteen Names A B C D E F G H I J K L",
            )
...
class TestAccountEligibility:
    ...
    @pytest.mark.skip(
        reason="Checking for affiliations temp. patched out (ocflib PR 140)",
    )
...
class TestPasswordEncryption:
    @pytest.mark.parametrize(
        "password",
        ["hello world", "hunter2", "mock_send_mail_user.assert_called_once_with",],
    )
...
class TestValidateEmail:
    ...
    @pytest.mark.parametrize(
        "email", ["", "@", "hello@hello", "some kinda email@gmail.com",],
    )
...
class TestValidatePassword:
    @pytest.mark.parametrize(
        "password",
        ["correct horse battery staple", "pogjpaioshfoasdfnlka;sdfi;sagj", "p@ssw0rd",],
    )
...
class TestValidateRequest:
    def test_valid_request(
        ...
        assert validate_request(
            fake_new_account_request, fake_credentials, session,
        ) == ([], [])
    ...
    def test_invalid_request_warning(
        ...
        errors, warnings = validate_request(
            fake_new_account_request._replace(**attrs), fake_credentials, session,
        )
        assert warnings

    @pytest.mark.parametrize("attrs", [{"user_name": "ckuehl"},])
    ...
    def test_invalid_request_already_submitted(
        ...
        with mock.patch(
            "ocflib.account.submission.username_pending", return_value=True,
        ):
            errors, warnings = validate_request(
                fake_new_account_request, fake_credentials, session,
            )
        ...
        with mock.patch(
            "ocflib.account.submission.user_has_request_pending", return_value=True,
        ):
            errors, warnings = validate_request(
                fake_new_account_request, fake_credentials, session,
            )
...
class TestCreateAccount:
    @pytest.mark.parametrize(
        "is_group,calnet_uid,callink_oid,expected",
        [
    ...
    def test_create(
        ...
        with mock.patch(
            "ocflib.account.creation.create_kerberos_principal_with_keytab",
        ) as kerberos, mock.patch(
            "ocflib.account.creation.get_kerberos_principal_with_keytab",
            return_value=None,
        ) as kerberos_get, mock.patch(
            "ocflib.account.creation.create_ldap_entry",
        ) as ldap, mock.patch(
            "ocflib.account.creation.create_home_dir",
        ) as home_dir, mock.patch(
            "ocflib.account.creation.ensure_web_dir",
