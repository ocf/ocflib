...
class TestNameByCalNetUID:
    ...
    def test_name_by_calnet_uid(self, attrs, expected):
        ...
        with mock.patch("ocflib.account.search.user_attrs_ucb", return_value=attrs):
            assert name_by_calnet_uid(0) == expected

    @pytest.mark.parametrize(
        "uid,expected", [(TEST_PERSON_CALNET_UID, TEST_PERSON_NAME), (9999999, None),],
    )
...
class TestCalNetUIDsByName:
    @pytest.mark.parametrize(
        "name,expected", [(TEST_PERSON_NAME, [TEST_PERSON_CALNET_UID]), ("~~~", []),],
    )
