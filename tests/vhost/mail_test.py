...
def _normalize_times(addrs):
    return {addr._replace(last_updated=None) for addr in addrs}
...
def test_get_forwarding_addresses_forward_to_parsing(
    forward_to, expected, example_vhost,
):
...
def test_add_and_remove_addresses(
    mysql_connection, example_vhost, another_example_vhost,
):
    ...
    assert _normalize_times(
        example_vhost.get_forwarding_addresses(mysql_connection),
    ) == {addr1}
    ...
    example_vhost.remove_forwarding_address(
        mysql_connection, "ckuehl@dev-vhost.ocf.berkeley.edu",
    )
    assert _normalize_times(
        example_vhost.get_forwarding_addresses(mysql_connection),
    ) == {addr2}
    ...
    example_vhost.remove_forwarding_address(
        mysql_connection, "@dev-vhost.ocf.berkeley.edu",
    )
    assert example_vhost.get_forwarding_addresses(mysql_connection) == set()
...
def test_mail_forwarding_address_is_wildcard():
    ...
    assert not MailForwardingAddress(
        "bob@vhost.com", None, frozenset(), None,
    ).is_wildcard
